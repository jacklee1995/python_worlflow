...
from workflow.steps.aggregate_step import AggregateStep
...
def aggregate_command(task_name: Optional[str] = None, mode: str = "development"):
    ...
    aggregate_step.set_destination(Destination("data/output.csv"))

    # 设置聚合参数
    aggregate_step.set_group_by(["category"])
    aggregate_step.set_aggregations({"sales": "sum"})
