...
class CopyStep(StepInterface):
    ...
    def __init__(self, name: str = 'CopyStep', description: str = 'Copy files or directories'):
        """
        初始化CopyStep。
    ...
    async def execute(self) -> None:
        ...
        for source in self.sources:
            src_path = await source.get_path()
            dest_path = self.output.get_path()

            if await source.is_file():
                shutil.copy(src_path, dest_path)
            elif await source.is_dir():
